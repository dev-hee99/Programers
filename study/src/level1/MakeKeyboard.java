 ...
 * <p>
 * 이 휴대폰 자판은 키의 개수가 1개부터 최대 100개까지 있을 수 있으며, 특정 키를 눌렀을 때 입력되는 문자들도 무작위로 배열되어 있습니다. 또, 같은 문자가 자판 전체에 여러 번 할당된 경우도 있고,
...
public class MakeKeyboard {
    ...
    public static void main(String[] args) {
        ...
        Arrays.stream(solution(keymap, target)).forEach(i -> System.out.println(i));
    }
    ...
    private static int[] solution(String[] keymap, String[] targets) {
        ...
        // 키맵에 있는 문자와 해당 인덱스를 담을 Map객체
        HashMap<Character, Integer> keyHashMap = new HashMap<>();

        List<String> keyMapList = Arrays.asList(keymap);

        for (int i = 0; i < keyMapList.size(); i++) {
            char[] keyArray = keyMapList.get(i).toCharArray();
            for (int j = 0; j < keyArray.length; j++) {
                char key = keyArray[j];
                int keyNum = j + 1;
                // key가 존재 하지 않으면 값을 넣어준다.
                if (!keyHashMap.containsKey(keyArray[j])) {
                    keyHashMap.put(key, keyNum);
                }else{ // key가 존재 하면 해당 키 인덱스 값과 비교하여 더 작은 값을 재할당 해준다.
                    if(keyHashMap.get(key) > j + 1){
                        keyHashMap.put(key, keyNum);
                    }
                }
            }
        ...
        // 포함하는 키가 없으면 -1 리턴, 포함하는 키가 있으면 해당 키 값의 합을 구한다.
        for (int i = 0; i < targets.length; i++) {
            int count = 0;
            char[] targetArray = targets[i].toCharArray();
            for (int j = 0; j < targetArray.length; j++) {
                char key = targetArray[j];
                if(!keyHashMap.containsKey(key)){
                    count = -1;
                    break;
                }
                count += keyHashMap.get(key);
            }
            answer[i] = count;
        }
