...
public class JustThatSong {
    static final String m = "ABCDEFG";
    static final String[] musicinfos = {"12:00,12:14,HELLO,CDEFGAB", "13:00,13:05,WORLD,ABCDEF"};
    ...
    public static String solution(String m, String[] musicinfos){
        ...
        for (String musicInfo : musicinfos) {
            ...
            //악보
            String musicCode = changeNote(info[3]);

            StringBuilder sb = new StringBuilder();
            //재생시간이 악보보다 길 경우
            if(playTime > musicCode.length()){
                for(int i=0;i<playTime/musicCode.length();i++) {
                    sb.append(musicCode);
                }
                    sb.append(musicCode.substring(0, playTime % musicCode.length()));
            //짧을 경우
            }else{
                    sb.append(musicCode.substring(0,playTime));
            }
            if(sb.indexOf(m)!=-1 && playTime > maxPlayTime){
               answer = musicTitle;
    ...
    public static String changeNote(String note) {
        //C#:H D#:I F#:J G#:K  A#:L
